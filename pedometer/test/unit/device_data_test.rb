...
class DeviceDataTest < Test::Unit::TestCase
  ...
  def test_create_accelerometer_data
    ...
    assert_equal [{:x=>0.123, :y=>-0.123, :z=>5.0, :xg=>0, :yg=>0, :zg=>0},
                  ...
                  {:x=>-0.219, :y=>0.024, :z=>0.967, :xg=>0.007, :yg=>-0.023, :zg=>0.033}], device_data.parsed_data
    assert_equal [0.0, 0.0249, 0.0298], device_data.dot_product_data
    assert_equal [0, 0, -0.0015], device_data.filtered_data
  end
  ...
  def test_create_gravity_data
    ...
    assert_equal [-24.9283, 0.3663, 6.92], device_data.dot_product_data
    assert_equal [0, 0, -1.7824], device_data.filtered_data
  end

...
class AnalyzerTest < Test::Unit::TestCase
  ...
  def test_split_on_threshold
    parser = Parser.new(File.read('test/data/female/walking-10-g-1.txt'))
    analyzer = Analyzer.new(parser)
  ...
  def test_detect_edges_false_step
    parser = Parser.new(File.read('test/data/female/walking-1-g-false-step.txt'))
    analyzer = Analyzer.new(parser)
  ...
  def test_measure_time_minutes
    ...
    parser = Parser.new(1000.times.inject('') {|a| a+='1,1,1;';a})
    analyzer = Analyzer.new(parser, user)    
    analyzer.measure_time

...
\aosasectii{From Archaeology to
Databases}\label{from-archaeology-to-databases}
...
\item
  Each artifact has a `color' attribute (where white means that no
  update was made).
\item
  Black arrows denote a change in color between layers (e.g., from
  c.color @t2 to c.color @t0).
\item
  Light blue arrows are arbitrary relationships of interest between
  artifacts (e.g., from `b' to `d').
\end{aosaitemize}
...
\item
  The second level stores the related attribute-names (the greenish
  area).
\item
...
the following five entities is visualized in the table below (the color
coding follows the color coding of Figure 2 and Figure 3).

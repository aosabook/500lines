...
crucial here. Older data does not ``disappear''---it is buried, and can
be revealed again by exposing an older layer. Conversely, updating data
...
results of which are stored in \texttt{layer-with-updated-storage}.
...
\item
  ...
  the \texttt{partial}-ed \newline \texttt{update-entry-in-index} with
...
\item
  ...
  (like the \texttt{WHERE} clause in a SQL query).
\item
  ...
  projected into the results (like the \texttt{SELECT} clause in a SQL
  query).
\end{aosaitemize}
...
We saw in the previous phase that our query plan ends by calling
\texttt{single-index-query-plan}. This function will:

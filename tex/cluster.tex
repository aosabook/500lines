...
\item
  ...
  performs a \newline \texttt{Prepare}/\texttt{Promise} phase for ballot
  number 2.
\item
...
The idea is that each instance of Paxos (slot) uses the view from
$\alpha$ slots earlier. This allows the cluster to work on, at most,
$\alpha$ slots at any one time, so a very small value of $\alpha$ limits

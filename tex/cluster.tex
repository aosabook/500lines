...
Part-Time Parliament''\footnote{Leslie Lamport. The part-time
  parliament. ACM Transactions on Computer Systems, 16(2):133--169, May
  1998.}. Lamport's paper has a great deal more detail than we will get
into here, and is a fun read. The references at the end of the chapter
describe some extensions of the algorithm that we have adapted in this
implementation.
...
Made Simple''\footnote{Leslie Lamport. Paxos Made Simple. ACM SIGACT
...
Moderately Complex''\footnote{Renesse - ``Paxos Made Moderately
  Complex'' (the origin of the role names)}. ``Paxos Made
Live''\footnote{Chandra, Griesemer, and Redstone - ``Paxos Made Live -
  An Engineering Perspective'' (regarding snapshots, in particular)} was
helpful regarding snapshots in particular, and ``Paxos Made
Practical''\footnote{Mazieres - ``Paxos Made Practical'' (view changes,
  although not of the type described here)} described view changes
(although not of the type described here.) Liskov's ``From Viewstamped
Replication to Byzantine Fault Tolerance''\footnote{Liskov - ``From
  Viewstamped Replication to Byzantine Fault Tolerance'' (another,
  different look at view changes)} provided yet another perspective on

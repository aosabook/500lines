...
download(TorrentFile) ->
    ...
    case catch gproc:lookup_local_name(Info#torrent_info.info_hash) of
        undefined ->
            plain_fsm:start_opt(?MODULE, fun() -> init(Info) end, 60*1000, []);
        PID ->
            {ok, PID}
    end.
    ...
    case Missing of
        ...
        _  -> case track(started, State) of
                  {ok, Interval, Peers} ->
                      ...
                      {ok, #client{ peer_id=ClientID }} = torrent_client:client_info(),
...
main_loop(State=#state{ info=Info, peers=Peers, have=Have, missing = Missing }) ->
    ...
    receive
        ...
        {'DOWN', _, process, PID, Info} ->
            main_loop(State#state{ peers=lists:delete(PID, Peers) });

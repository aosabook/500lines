...
Dagoba.make_fun = function (name) {
    return function () {
        return this.add([name].concat(Array.prototype.slice.apply(arguments)));
    };
};
...
Dagoba.query = function (graph) {
    ...
    query.gremlins = [];
    // array of gremlins for each step
    query.state = [];
    // array of state for each step
    return query;
};
//lalal
Dagoba.Query.run = function () {
    ...
    function stepper(step_index, gremlin) {
        ...
        if (!Dagoba.Funs[step[0]])
            return Dagoba.onError('Unrecognized function call: ' + step[0]) || {};
        return Dagoba.Funs[step[0]](graph, step.slice(1) || {}, gremlin || {}, state[step_index] || {});
    }
    function eat_result(step_index, result$2) {
        state[step_index] = result$2.state;
        eat_gremlins(step_index, result$2);
    }
    function eat_gremlins(step_index, result$2) {
        gremlins = gremlins.concat((result$2.stay || []).map(gremlin_boxer(step_index)) || []).concat((result$2.go || []).map(gremlin_boxer(step_index + 1)) || []);
    }
    // add final queue item
    this.add(['collector']);
    // the collector just takes gremlins out of circulation
    ...
    this.result = collection.map(function (gremlin) {
        return gremlin.vertex;
    });
...
methods.forEach(function (name) {
    Dagoba.Query[name] = Dagoba.make_fun(name);
});
Dagoba.Funs = {
...
Dagoba.addhook = function (type, callback) {
    if (!Dagoba.hooks[type])
        Dagoba.hooks[type] = [];
    Dagoba.hooks[type].push(callback);
};
Dagoba.firehooks = function (type, query) {
    var args = [].slice.call(arguments, 2);
    return ((Dagoba.hooks || {})[type] || []).reduce(function (acc, callback) {
        return callback.apply(query, acc);
    }, args);
};
Dagoba.addhook('postquery', Dagoba.uniqueify = function (results) {
    // THINK: should we inline this and merge&count in the gremlins?
    return [results.filter(function (item, index, array) {
            return array.indexOf(item) == index;
        })];
});
Dagoba.addhook('postquery', Dagoba.cleanclone = function (results) {
    // THINK: do we always want this?
...
Dagoba.graph = function () {
    ...
    graph.vertices = [];
    // can't stick these on the prototype or they'll be shared
    graph.edges = [];
    graph.vertexIndex = {};
    return graph;
};
...
Dagoba.Graph.searchVertices = function (obj) {
    return this.vertices.filter(function (vertex) {
        return Object.keys(obj[0]).reduce(function (acc, key) {
            return acc && obj[0][key] == vertex[key];
        }, true);
    });
};
Dagoba.Graph.findEdgeById = function (edge_id) {
    return this.edges.first(function (edge) {
...
Dagoba.Graph.findOutEdges = function (vertex) {
    ...
    return this.edges.filter(function (edge) {
        return edge._out == vertex._id;
    });
};
Dagoba.Graph.findInEdges = function (vertex) {
    return vertex._in;
    return this.edges.filter(function (edge) {
        return edge._in == vertex._id;
    });
};
Dagoba.filterThings = function (arg) {
    return function (thing) {
        return !arg ? true : arg + '' === arg ? thing._label == arg : Array.isArray(arg) ? !!~arg.indexOf(thing._label) : Dagoba.objFilter(thing, arg);
    };
};

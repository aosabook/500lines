(ns core.manage
  "Management of db creation and db connections. This module allows acquire new db or reset an existing one, as well as get the db value from a connection"
  (:use core.constructs))

(def __ALL-DBS__ (atom {}))

(defn _get-db  [dbs db-name] (if (db-name dbs)  dbs  (assoc dbs db-name (make-db))))

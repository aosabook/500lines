...
def main(chapters=[], epub=False, pdf=False, html=False, mobi=False,
         pandoc_epub=False):
    ...
    chapter_dirs = '''blockcode ci cluster contingent crawler dagoba data-store
        event-web-framework flow-shop functionalDB image-filters interpreter
        modeller objmodel ocr pedometer same-origin-policy sampler spreadsheet
        static-analysis template-engine web-server'''.split()

    if chapters:
        chapter_dirs = [chapter_dir for chapter_dir in chapter_dirs
                        if chapter_dir in chapters]

    chapter_markdowns = ['./{0}/{0}.markdown'.format(chapter_dir)
    ...
    process_chapters = [chapter_markdown for chapter_markdown, process
                        in zip(chapter_markdowns, chapter_markdowns_exist)
                        if process == 0]

    chapter_names = [getbasename(chapter) for chapter in chapter_dirs]

    image_paths = ['./{0}/{0}-images'.format(chapter) for chapter
                   in '''blockcode ci cluster contingent crawler data-store
                         flow-shop functionalDB image-filters interpreter
                         modeller objmodel ocr pedometer same-origin-policy
                         sampler spreadsheet web-server'''.split()]
...
def build_epub(chapter_markdowns, pandoc_epub):
    ...
    temp = ('python _build/preprocessor.py --chapter {chapnum} '
            '--output=epub/{basename}.markdown.1 --latex {md}')
    ...
    os.chdir('epub')
    temp = ('../_build/increaseheaders.sh {basename}.markdown.1 '
            '{basename}.markdown {chapnum}')
    for i, markdown in enumerate(chapter_markdowns):
        basename = os.path.splitext(os.path.split(markdown)[1])[0]
        run(temp.format(md=markdown, basename=basename, chapnum=i + 1))
    pandoc_path = 'pandoc'
    cmd = ('{pandoc} --chapters -S -f markdown+mmd_title_block '
           '--highlight-style=kate -o 500L.epub epubtitle.txt '
           'introduction.markdown {markdowns}')
    if pandoc_epub:
...
#    print 'zip 500L.epub META-INF mimetype nav.xhtml toc.ncx stylesheet.css content.opf ' + output  # noqa
#    sp.check_output(
#        'zip 500L.epub META-INF mimetype nav.xhtml toc.ncx stylesheet.css content.opf ' + output,  # noqa
#        shell=True)
...
def build_html(chapter_markdowns):
    ...
    temp = ('python _build/preprocessor.py --chapter {chap} --html-refs '
            '--output={md}.1 --latex {md}')
    temp2 = ('pandoc --csl=minutiae/pdf/ieee.csl --mathjax -t html -f '
             'markdown+citations -o html/content/pages/{basename}.md {md}.1')
...
def _pandoc_cmd(chapter_markdown):
    ...
    temp = ('{pandoc} -V chaptertoken={chaptertoken} -t latex --chapters -S '
            '-f markdown+mmd_title_block+tex_math_dollars '
            '--template=tex/chaptertemplate.tex --no-highlight '
            '-o tex/{basename}.tex.1 tex/{md}')
    basename = getbasename(chapter_markdown)
    result = temp.format(pandoc=pandoc_path, basename=basename,
                         md=chapter_markdown, chaptertoken='s:' + basename)
    return result


def preprocessor_command(chapter_markdown):
    temp = ('python _build/preprocessor.py --output=tex/{basename}.markdown '
            '--markdown {md}')
    basename = getbasename(chapter_markdown)
...
def postprocessor_command(basename):
    temp = ('python _build/postprocessor.py --output=tex/{basename}.tex '
            'tex/{basename}.tex.1')
...
def pandoc_cmd(chapter_markdown):
    ...
    print(result.std_err if result.status_code != 0 else result.std_out)
    new_chapter_markdown = basename + '.markdown'
    result = envoy.run(_pandoc_cmd(new_chapter_markdown))
    print(result.std_err if result.status_code != 0 else result.std_out)
    return envoy.run(postprocessor_command(basename))
...
if __name__ == '__main__':
    ...
    main(chapters=args.chapters, epub=args.epub, pdf=args.pdf,
         html=args.html, mobi=args.mobi, pandoc_epub=args.pandoc_epub)

...
class Tests(utils.ComponentTestCase):
    ...
    def setUp(self):
        ...
        self.commander_id = CommanderId(address=self.node.address, slot=self.slot, proposal=self.proposal)
        self.ballot_num = Ballot(91, 82)
        self.cmd = commander.Commander(self.member, leader=self.leader, ballot_num=self.ballot_num,
                                       slot=self.slot, proposal=self.proposal,
                                       commander_id=self.commander_id, peers=['p1', 'p2', 'p3'])
        self.accept_kwargs = dict(commander_id=self.commander_id, ballot_num=self.ballot_num, slot=self.slot,
    ...
    def test_retransmit(self):
        ...
        self.node.fake_message('ACCEPTED', commander_id=self.commander_id, acceptor='p2',
                                           ballot_num=self.ballot_num)
        self.node.tick(ACCEPT_RETRANSMIT)
        self.assertMessage(['p1', 'p3'], 'ACCEPT', **self.accept_kwargs)
        self.node.tick(ACCEPT_RETRANSMIT)
        self.assertMessage(['p1', 'p3'], 'ACCEPT', **self.accept_kwargs)
        self.node.fake_message('ACCEPTED', commander_id=self.commander_id, acceptor='p1',
                                           ballot_num=self.ballot_num)

        # quorum (3/2+1 = 2) reached
        self.assertMessage(['p1', 'p2', 'p3'], 'DECISION', slot=self.slot, proposal=self.proposal)

        self.leader.commander_finished.assert_called_with(self.commander_id, self.ballot_num, False)
        self.assertTimers([])
    ...
    def test_wrong_commander_id(self):
        ...
        self.cmd.start()
        self.assertMessage(['p1', 'p2', 'p3'], 'ACCEPT', **self.accept_kwargs)
        other_commander_id = CommanderId(address='OTHER', slot=self.slot, proposal=self.proposal)
        self.node.fake_message('ACCEPTED', commander_id=other_commander_id, acceptor='p1',
                                           ballot_num=self.ballot_num)
        self.node.tick(ACCEPT_RETRANSMIT)
        self.assertMessage(['p1', 'p2', 'p3'], 'ACCEPT', **self.accept_kwargs)  # p1 still in the list
    ...
    def test_preempted(self):
        ...
        self.leader.commander_finished.assert_called_with(self.commander_id, other_ballot_num, True)
        self.assertTimers([])
        self.assertUnregistered()
